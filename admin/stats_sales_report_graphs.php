...
for ($i = 0; $i < $report->size; $i++) {
  ...
  switch ($sales_report_view) {
      case statsSalesReportGraph::YEARLY_VIEW:
          ...
          break;
      case statsSalesReportGraph::MONTHLY_VIEW:
          echo $zcDate->output('MMM', $report->info[$i]['startDates']);
            ...
            <tbody>
                ...
                <tr class="dataTableRow">
                  <td class="dataTableContent">
                      ...
                      switch ($sales_report_view) {
                          case statsSalesReportGraph::HOURLY_VIEW:
                              echo $zcDate->output('HH', $report->info[$i]['startDates']) . ' - ' . $zcDate->output('HH', $report->info[$i]['endDates']) . (($i === 0) ? ' ' . $zcDate->output(DATE_FORMAT_SHORT, $report->info[$i]['startDates']) : '');
                              break;
                          case statsSalesReportGraph::DAILY_VIEW:
